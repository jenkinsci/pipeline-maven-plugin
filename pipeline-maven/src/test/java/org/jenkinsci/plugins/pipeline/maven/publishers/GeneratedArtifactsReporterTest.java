...
public class GeneratedArtifactsReporterTest {
    ...
    public void before() throws Exception {
        {
            ...
            InputStream inMacOSX =
                    Thread.currentThread().getContextClassLoader().getResourceAsStream(mavenSpyLogsOnMacOSXPath);
            mavenSpyLogsOnMacOSX =
                    DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(inMacOSX);
        }
        {
            String mavenSpyLogsOnWindowsPath = "org/jenkinsci/plugins/pipeline/maven/maven-spy-windows.xml";
            InputStream inWindows =
                    Thread.currentThread().getContextClassLoader().getResourceAsStream(mavenSpyLogsOnWindowsPath);
            mavenSpyLogsOnWindows =
                    DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(inWindows);
        }
    }
    ...
    public void testListArtifactsMacOSX() throws Exception {
        List<MavenArtifact> mavenArtifacts =
                XmlUtils.listGeneratedArtifacts(this.mavenSpyLogsOnMacOSX.getDocumentElement(), false);
        ...
        Element projectStartedElt = XmlUtils.getExecutionEvents(
                        this.mavenSpyLogsOnMacOSX.getDocumentElement(), "ProjectStarted")
                .get(0);
        String workspace =
                XmlUtils.getUniqueChildElement(projectStartedElt, "project").getAttribute("baseDir");

        String pomPathInWorkspace =
                XmlUtils.getPathInWorkspace(pomArtifact.getFile(), new FilePath(new File(workspace)));
        ...
        assertThat(mavenArtifact.getFile())
                .isEqualTo("/path/to/spring-petclinic/target/spring-petclinic-" + PETCLINIC_VERSION + ".jar");
    ...
    public void testListArtifactsWindows() throws Exception {
        List<MavenArtifact> mavenArtifacts =
                XmlUtils.listGeneratedArtifacts(this.mavenSpyLogsOnWindows.getDocumentElement(), false);
        System.out.println(mavenArtifacts);
        assertThat(mavenArtifacts).hasSize(2);
        ...
        Element projectStartedElt = XmlUtils.getExecutionEvents(
                        this.mavenSpyLogsOnWindows.getDocumentElement(), "ProjectStarted")
                .get(0);
        String workspace =
                XmlUtils.getUniqueChildElement(projectStartedElt, "project").getAttribute("baseDir");
        ...
        assertThat(mavenArtifact.getArtifactId()).isEqualTo("spring-petclinic");
        assertThat(mavenArtifact.getFile())
                .isEqualTo("C:\\path\\to\\spring-petclinic\\target\\spring-petclinic-" + PETCLINIC_VERSION + ".jar");
        assertThat(mavenArtifact.getFileName()).isEqualTo("spring-petclinic-" + PETCLINIC_VERSION + ".jar");
    }
    ...
    public void testListAttachedArtifactsMacOSX() throws Exception {
        List<MavenArtifact> mavenArtifacts =
                XmlUtils.listGeneratedArtifacts(this.mavenSpyLogsOnMacOSX.getDocumentElement(), true);
        ...
        assertThat(mavenArtifact.getFile())
                .isEqualTo("/path/to/spring-petclinic/target/spring-petclinic-" + PETCLINIC_VERSION + "-sources.jar");
    }

    @Test
    public void testListAttachedArtifactsWindows() throws Exception {
        List<MavenArtifact> mavenArtifacts =
                XmlUtils.listGeneratedArtifacts(this.mavenSpyLogsOnWindows.getDocumentElement(), true);
        assertThat(mavenArtifacts).hasSize(3);
        MavenArtifact mavenArtifact = mavenArtifacts.get(2); // 1st is pom, 2nd is jar, 3rd is sources
        System.out.println(mavenArtifacts);
        assertThat(mavenArtifact.getArtifactId()).isEqualTo("spring-petclinic");
        assertThat(mavenArtifact.getClassifier()).isEqualTo("sources");
        assertThat(mavenArtifact.getFile())
                .isEqualTo("C:\\path\\to\\spring-petclinic\\target\\spring-petclinic-" + PETCLINIC_VERSION
                        + "-sources.jar");
    }
}
