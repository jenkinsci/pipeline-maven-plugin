...
*/
@Disabled(
        "TODO adapt to https://github.com/jenkinsci/junit-plugin/pull/155 (mock JunitTestResultStorageConfiguration.get) or stop using mock frameworks")
public class JunitTestsPublisherTest {
    ...
    public void test_karma_plugin() throws Exception {
        ...
        calendar.setTimeInMillis(
                new File("src/test/resources/org/jenkinsci/plugins/pipeline/maven/karma-reports/TEST-karma.xml")
                        .lastModified());
        final StepContext context = mock(StepContext.class);
        final Run run = mock(Run.class);
        final Launcher launcher = mock(Launcher.class);
