...
public class DependencyGraphTest extends AbstractIntegrationTest {
    ...
    public void verify_downstream_multi_branch_pipeline_trigger() throws Exception {
        ...
        Future<WorkflowRun> mavenJarPipelineMasterPipelineSecondRunFuture = mavenJarPipelineMasterPipeline.scheduleBuild2(0,
                new CauseAction(new Cause.RemoteCause("127.0.0.1", "junit test")));
    ...
    public void verify_osgi_bundle_recorded_as_bundle_and_as_jar() throws Exception {
        ...
        Iterable<String> matchingArtifactTypes = generatedArtifacts.stream()
                .filter(input -> input != null && input.getGroupId().equals("jenkins.mvn.test.bundle") && input.getArtifactId().equals("print-api")
                        && input.getVersion().equals("0.0.1-SNAPSHOT"))
                .map(MavenArtifact::getType).collect(Collectors.toList());
    ...
    public void verify_downstream_pipeline_triggered_on_parent_pom_build() throws Exception {
        ...
        Iterable<String> matchingArtifactTypes = generatedArtifacts.stream().filter(input -> input != null && input.getGroupId().equals("com.example")
                && input.getArtifactId().equals("my-pom") && input.getVersion().equals("0.1-SNAPSHOT")).map(MavenArtifact::getType)
    ...
    public void verify_downstream_pipeline_triggered_on_jar_having_parent_pom_dependency() throws Exception {
        ...
        loadSourceCodeInGitRepository(this.gitRepoRule, "/org/jenkinsci/plugins/pipeline/maven/test/test_maven_projects/maven_pom_project/");
        System.out.println("downstreamArtifactRepoRule: " + downstreamArtifactRepoRule);
        loadSourceCodeInGitRepository(this.downstreamArtifactRepoRule,
    ...
    public void verify_docker_downstream_simple_pipeline_trigger() throws Exception {
        ...
        assertThat(mavenDockerBasePipelineLastRun.getNumber()).isEqualTo(mavenDockerBasePipelineFirstRun.getNumber() + 1);
    ...
    public void verify_deployfile_downstream_simple_pipeline_trigger() throws Exception {
        PipelineGraphPublisher publisher = GlobalPipelineMavenConfig.get().getPublisherOptions().stream()
                .filter(p -> PipelineGraphPublisher.class.isInstance(p)).findFirst().map(p -> (PipelineGraphPublisher) p).get();
        ...
        System.out.println(
                "build-deployfile-baseLastBuild: " + mavenDeployFileBasePipelineLastRun + " caused by " + mavenDeployFileBasePipelineLastRun.getCauses());

        assertThat(mavenDeployFileBasePipelineLastRun.getNumber()).isEqualTo(mavenDeployFileBasePipelineFirstRun.getNumber() + 1);
