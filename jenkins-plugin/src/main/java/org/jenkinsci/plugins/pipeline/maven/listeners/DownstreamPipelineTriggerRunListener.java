    ...
    public void onCompleted(WorkflowRun upstreamBuild, @Nonnull TaskListener listener) {
        ...
        for (Map.Entry<MavenArtifact, SortedSet<String>> entry: downstreamPipelinesByArtifact.entrySet()) {
            ...
            for (String downstreamPipelineFullName : downstreamPipelines) {
                ...
                Map<MavenArtifact, SortedSet<String>> downstreamDownstreamPipelinesByArtifact = daoHelper.listDownstreamJobsByArtifact(downstreamPipelineFullName, downstreamBuildNumber);
                ...
                if (!downstreamPipeline.isBuildable()) {
                    if (LOGGER.isLoggable(Level.FINER)) {
                        listener.getLogger().println("[withMaven] downstreamPipelineTriggerRunListener - Skip triggering of non buildable (disabled: " + downstreamPipeline.isDisabled() +
                                ", isHoldOffBuildUntilSave: " + downstreamPipeline.isHoldOffBuildUntilSave() + ") downstream pipeline " + downstreamPipeline.getFullName() +
